...
class MainActivity : FlutterActivity() {
    ...
    private val RESULT_USAGE_PERMISSION = 100;
    private val RESULT_OVERLAY_PERMISSION = 101;

    private var fluttterResult: MethodChannel.Result? = null
            ...
            when {
                ...
                call.method.equals("askUsageStatsPermission") -> {
                    ...
                    if (!isAccessGranted()) {
                        ...
                        startActivityForResult(intent, RESULT_USAGE_PERMISSION)
                    }
                }

                call.method.equals("saveEventData") -> {
                    fluttterResult = result
                    val args = call.arguments as HashMap<String, String>
                    val eventData: String = args.getValue("event_data")
                    println("saveEventData ${eventData} ----- ARGS")
                    val editor: SharedPreferences.Editor = saveAppData!!.edit()
                    editor.putString("event_data", eventData)
                    editor.apply()
                    result.success("Success")
//                    if (!isAccessGranted()) {
//                        val intent = Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS)
...
//                    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RESULT_USAGE_PERMISSION) {
            Log.d("TAG", "onActivityResult: resultcode ${resultCode}");
            if (resultCode == Activity.RESULT_OK) {
                fluttterResult?.success(true)
            } else {
                fluttterResult?.success(false)
            }
        } else if (requestCode == RESULT_OVERLAY_PERMISSION) {
            Log.d("TAG", "onActivityResult: resultcode ${resultCode}");
            if (resultCode == Activity.RESULT_OK) {
                fluttterResult?.success(true)
            } else {
...
//        val mContentView = RemoteViews(packageName, R.layout.list_view)
    ...
    private fun checkOverlayPermission(): Boolean {
        if (!Settings.canDrawOverlays(this)) {
            ...
            startActivityForResult(myIntent, RESULT_OVERLAY_PERMISSION)
        } else {
            fluttterResult?.success(true)
        }
